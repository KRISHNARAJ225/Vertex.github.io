...
public class RoleController {
    ...
    public ResponseEntity<ApiResponse> addRole(@RequestBody Role role) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.CREATED.value(), "Role Created", service.save(role)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse> updateRole(@PathVariable Long id, @RequestBody Role role) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Role Updated", service.update(id, role)));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse> deleteRole(@PathVariable Long id) {
        service.delete(id);
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Role Deleted", null));
    }

    @PostMapping("/{roleId}/permissions")
    public ResponseEntity<ApiResponse> assignPermissions(@PathVariable Long roleId, @RequestBody List<Long> permissionIds) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Permissions Assigned", service.assignPermissions(roleId, permissionIds)));
    }
}
