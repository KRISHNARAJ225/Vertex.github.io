...
public class OrderController {
    ...
    public ResponseEntity<ApiResponse> updateStatus(@PathVariable Long id, @RequestParam String status) {
        ...
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Order Status Updated", service.updateStatus(id, status.toUpperCase())));
    }
    ...
    public ResponseEntity<ApiResponse> updatePaymentStatus(@PathVariable Long id, @RequestParam String paymentStatus) {
        ...
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Payment Status Updated", service.updatePaymentStatus(id, paymentStatus.toUpperCase())));
    }
    ...
    public ResponseEntity<ApiResponse> getOrdersByCustomer(@PathVariable Long customerId) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Success", service.findByCustomer(customerId)));
    }

    @GetMapping("/status/{status}")
    public ResponseEntity<ApiResponse> getOrdersByStatus(@PathVariable String status) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Success", service.findByStatus(status)));
    }
    ...
    public ResponseEntity<Resource> getQR(@PathVariable String orderCode) {
        ...
        if (filePath == null || filePath.isEmpty()) throw new RuntimeException("QR not generated");

        Path path = Paths.get(filePath);
        if (!Files.exists(path)) throw new RuntimeException("QR file not found");
