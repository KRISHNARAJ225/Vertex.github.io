...
import com.Product.Shop.Service.InvoiceService;
...
public class InvoiceController {
    ...
    public ResponseEntity<ApiResponse> getAllInvoices(
        ...
        return ResponseEntity.ok(
            new ApiResponse(HttpStatus.OK.value(), "Success", service.listAll(search, status, customerId, page, size, sortBy, sortDir)));
    }
    ...
    public ResponseEntity<ApiResponse> generateInvoice(@PathVariable String orderCode) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.CREATED.value(), "Invoice Generated", service.generateInvoice(orderCode)));
    }

    @GetMapping("/order/{orderCode}")
    public ResponseEntity<ApiResponse> getByOrderCode(@PathVariable String orderCode) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Success", service.getByOrderCode(orderCode)));
    }
    ...
    public ResponseEntity<ApiResponse> updateStatus(@PathVariable Long id, @RequestParam String status) {
        ...
        if (!validStatus.contains(status.toUpperCase())) {
            return ResponseEntity.badRequest()
                .body(new ApiResponse(HttpStatus.BAD_REQUEST.value(), "Invalid status. Allowed: PENDING, APPROVED, CANCELLED", null));
        }
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Invoice Status Updated", service.updateStatus(id, status)));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse> deleteInvoice(@PathVariable Long id) {
        service.delete(id);
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Invoice Deleted", null));
    }
}
