...
public class UserController {
    ...
    public ResponseEntity<ApiResponse> addUser(@RequestBody User user) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.CREATED.value(), "User Created", service.save(user)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse> updateUser(@PathVariable Integer id, @RequestBody User user) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "User Updated", service.update(id, user)));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse> deleteUser(@PathVariable Integer id) {
        service.delete(id);
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "User Deleted", null));
    }

    @PostMapping("/{userId}/roles")
    public ResponseEntity<ApiResponse> assignRoles(@PathVariable Integer userId, @RequestBody List<Long> roleIds) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Roles Assigned", service.assignRoles(userId, roleIds)));
    }
}
