...
import com.Product.Shop.Service.CustomerService;
...
public class CustomerController {
    ...
    public ResponseEntity<ApiResponse> addCustomer(@Valid @RequestBody CustomerRequestDto customer) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.CREATED.value(), "Customer Created", service.save(customer)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse> updateCustomer(@PathVariable Long id, @Valid @RequestBody CustomerRequestDto customer) {
        return ResponseEntity.ok(new ApiResponse(HttpStatus.OK.value(), "Customer Updated", service.update(id, customer)));
    }
